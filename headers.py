...
def load_data():
    ...
    for year in range(2021, 2025):  #2021 se 2024 tk loop chala rhy (for years)
        file_path = f"D:\DESKTOP\pfaiproj\Years\{year}.csv"#change path as you wish
...
def predict_outcome( playoff_threshold=2, win_rate_threshold=0.4):
    ...
    df = pd.read_csv(r"D:\DESKTOP\pfaiproj\Years\AllYears.csv")  #change path as you wish
...
def Winners_of_each_stage(df):
    ...
    top_3_teams = df.sort_values(by='Win Rate', ascending=False).head(5).reset_index()
...
def playersearch(name):
    csv_files = glob.glob(r"D:\DESKTOP\pfaiproj\Player Stats\*csv")  #change path as you wish
